...
export const PLATFORM_FEE_PCT = 0.10; // 10% comissão da plataforma
...
export function statusLabel(s: string): { label: string; tone: string } {
  const map: Record<string, { label: string; tone: string }> = {
    awaiting_payment: { label: "Aguardando pagamento", tone: "bg-yellow-500/15 text-yellow-400 border-yellow-500/30" },
    paid: { label: "Pago — aguardando entrega", tone: "bg-blue-500/15 text-blue-400 border-blue-500/30" },
    delivered: { label: "Entregue — em escrow", tone: "bg-purple-500/15 text-purple-400 border-purple-500/30" },
    released: { label: "Liberado", tone: "bg-green-500/15 text-green-400 border-green-500/30" },
    disputed: { label: "Em disputa", tone: "bg-orange-500/15 text-orange-400 border-orange-500/30" },
