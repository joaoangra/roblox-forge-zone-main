...
function WalletPage() {
  ...
  const { data: wallet } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () =>
      (await supabase.from("wallets").select("*").eq("user_id", user!.id).maybeSingle()).data,
  });
  ...
  async function requestWithdraw() {
    ...
    const { error } = await supabase
      .from("withdrawals")
      .insert({ user_id: user!.id, amount_cents: cents, pix_key: pix, pix_key_type: pixType });
    if (error) toast.error(error.message);
    else {
      toast.success("Solicitação enviada");
      setAmount("");
      setPix("");
      qc.invalidateQueries({ queryKey: ["my-wds"] });
    }
  }
          ...
          <Card>
            <CardContent className="p-5">
              ...
              <div className="text-2xl font-bold text-gradient-brand">
                {brl(wallet?.available_cents ?? 0)}
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-5">
              <div className="text-xs text-muted-foreground">Pendente (escrow)</div>
              <div className="text-2xl font-bold">{brl(wallet?.pending_cents ?? 0)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-5">
              <div className="text-xs text-muted-foreground">Bloqueado</div>
              <div className="text-2xl font-bold">{brl(wallet?.blocked_cents ?? 0)}</div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          <Card>
            <CardContent className="p-6 space-y-3">
              ...
              <Button onClick={requestWithdraw} className="w-full">
                Solicitar
              </Button>
              <p className="text-xs text-muted-foreground">
                Saques são processados manualmente pelo admin em até 24h úteis.
              </p>
            </CardContent>
          </Card>
