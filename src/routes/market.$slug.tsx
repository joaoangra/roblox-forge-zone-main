    ...
    queryFn: async () =>
      (
        await supabase
          .from("listings")
          .select(
            "*, category:marketplace_categories(name, slug), images:listing_images(*), questions:listing_questions(*)",
          )
          .eq("slug", slug)
          .maybeSingle()
      ).data,
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <div className="space-y-3">
                {l.questions?.length ? (
                  (
                    l.questions as Array<{ id: string; question: string; answer: string | null }>
                  ).map((q) => (
        ...
        <div className="space-y-4">
          ...
          <Card className="border-yellow-500/20 bg-yellow-500/5">
            <CardContent className="p-4 text-xs flex gap-2">
              ...
              <span>
                <strong>Nunca envie dinheiro fora da plataforma.</strong> Use sempre o chat do
                pedido — é a única forma de garantir suas evidências em caso de disputa.
              </span>
            </CardContent>
          </Card>
        </div>
