  ...
  head: () => ({
    meta: [
      { title: "Marketplace Roblox — Itens, Game Passes e Serviços" },
      {
        name: "description",
        content:
          "Compre itens Roblox, game passes e serviços de vendedores verificados com pagamento protegido por escrow.",
      },
    ],
  }),
...
function MarketIndex() {
  ...
  const { data: listings, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("listings")
        .select("*, category:marketplace_categories(slug, name)")
        .eq("status", "active")
      ...
      <section className="border-b border-white/5 bg-gradient-to-b from-primary/5 to-transparent">
        <div className="mx-auto max-w-7xl px-4 py-12">
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-3">
            <ShoppingBag className="h-4 w-4" /> Marketplace Roblox
          </div>
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-3">
            Compre com <span className="text-gradient-brand">segurança</span>
          </h1>
          <p className="text-muted-foreground max-w-2xl mb-6">
            Itens, game passes e serviços Roblox. Pagamento retido em escrow por 7 dias — só
            liberamos pro vendedor depois que você confirma que recebeu.
          </p>
          <div className="flex flex-wrap gap-2">
            <Button asChild className="bg-gradient-to-r from-primary to-accent text-white border-0">
              <Link to="/sell">
                <Plus className="h-4 w-4" /> Vender no Marketplace
              </Link>
            </Button>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <Card key={i} className="h-72 animate-pulse bg-white/5" />
            ))}
          </div>
          ...
          <Card className="p-12 text-center">
            ...
            <p className="text-muted-foreground">
              Nenhum anúncio encontrado. Seja o primeiro a vender!
            </p>
            <Button asChild className="mt-4">
              <Link to="/sell">Quero vender</Link>
            </Button>
          </Card>
        )}
      </div>
