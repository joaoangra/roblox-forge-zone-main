...
function ShopPage() {
  ...
  const { data: items } = useQuery({
    queryKey: ["shop-items"],
    queryFn: async () => {
      const { data } = await (supabase as any).from("shop_items").select("*").eq("active", true).order("price_points");
      return (data ?? []) as any[];
    },
  });
  ...
  async function buy(itemId: string, price: number) {
    ...
    if (pts.points < price) { toast.error("Pontos insuficientes"); return; }
    const { error } = await (supabase as any).from("shop_purchases").insert({ user_id: user.id, item_id: itemId, points_spent: price });
    if (error) toast.error(error.message);
    else { toast.success("Item adquirido!"); }
  }
    ...
    <PageShell>
      <div className="mx-auto max-w-7xl px-4 py-10">
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground mt-3 max-w-2xl mx-auto">Troque seus pontos BuxHub por descontos exclusivos e produtos especiais.</p>
          {user && (
          ...
          {(items ?? []).length > 0 ? (items ?? []).map((item: any) => (
            <Card key={item.id} className="border-white/10 bg-card/50 card-hover">
              <CardContent className="p-6 text-center">
                <div className="grid h-16 w-16 place-items-center rounded-xl bg-gradient-to-br from-pink-500/20 to-rose-500/20 mx-auto mb-4">
                  {item.discount_pct ? <Percent className="h-8 w-8 text-pink-400" /> : <Tag className="h-8 w-8 text-primary" />}
                </div>
                <h3 className="font-semibold mb-1">{item.name}</h3>
                <p className="text-sm text-muted-foreground mb-4">{item.description}</p>
                {item.discount_pct && (
                  <Badge className="bg-gradient-to-r from-pink-500 to-rose-500 border-0 mb-2">{item.discount_pct}% OFF</Badge>
                )}
                <div className="text-2xl font-bold text-gradient-brand mb-4">{item.price_points} pts</div>
                <Button onClick={() => buy(item.id, item.price_points)}
            ...
            <Card className="border-dashed border-white/10 col-span-full">
              <CardContent className="p-16 text-center text-muted-foreground">
                <Gift className="h-10 w-10 mx-auto mb-3 opacity-50" />Nenhum item disponível na loja no momento.
              </CardContent>
            </Card>
          )}
        </div>
