    ...
    <header className="sticky top-0 z-[9999] bg-background/95 backdrop-blur-sm glass">
        ...
        <div className="hidden md:flex items-center gap-2">
          {user ? (
            <>
              ...
              <Button asChild variant="ghost" size="sm">
                <Link to="/dashboard"><LayoutDashboard className="h-4 w-4" /> Painel</Link>
              </Button>
            ...
            <>
              ...
              <Button asChild size="sm" className="bg-gradient-to-r from-primary to-accent text-white border-0">
      ...
      {open && (
        <div className="md:hidden border-t border-white/10 px-4 py-3 space-y-1">
          {nav.map((n) => (
            <Link key={n.to} to={n.to} onClick={() => setOpen(false)} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-white/5">
              <n.icon className="h-4 w-4" /> {n.label}
            </Link>
          ))}
          {user ? (
            <>
              <Link to="/dashboard" onClick={() => setOpen(false)} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-white/5"><LayoutDashboard className="h-4 w-4" /> Painel</Link>
              {isAdmin && <Link to="/admin" onClick={() => setOpen(false)} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-white/5"><Shield className="h-4 w-4" /> Admin</Link>}
              <button onClick={async () => { await signOut(); setOpen(false); router.navigate({ to: "/" }); }} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-white/5"><LogOut className="h-4 w-4" /> Sair</button>
            </>
          ) : (
            <Link to="/auth" onClick={() => setOpen(false)} className="block rounded-md px-3 py-2 text-sm hover:bg-white/5">Entrar / Criar conta</Link>
          )}
        </div>
      )}
    </header>
